...
// Chat position is computed at runtime based on display size.
ImVec2 UISettings::m_chatPos = ImVec2(0.0f, 0.0f);
ImVec2 UISettings::m_chatPosNoRadar = ImVec2(0.0f, 0.0f);
...
void UISettings::Initialize(const ImVec2& display_size)
{
	...
	/* chat */
	const float chatMargin = 10.0f * m_scaleFactor.y;
	const float radarSize = display_size.x * 0.19f;
	const float radarOffset = -display_size.x * 0.03f;
	// Radar visivel: chat ao lado direito do radar
	m_chatPos = ImVec2(radarSize + radarOffset, chatMargin);
	// Radar oculto: chat ocupa a area do radar (canto superior esquerdo)
	m_chatPosNoRadar = ImVec2(chatMargin, chatMargin);
